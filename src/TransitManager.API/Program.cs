...
using System.Text;
using Microsoft.AspNetCore.Authorization; // <-- AJOUTER CE USING
using TransitManager.API.Authorization;     // <-- AJOUTER CE USING
var builder = WebApplication.CreateBuilder(args);
// --- CONFIGURATION DE LA CONNEXION À LA BASE DE DONNÉES ---
var connectionString = builder.Configuration.GetConnectionString("DefaultConnection");
builder.Services.AddDbContext<TransitContext>(options =>
    options.UseNpgsql(connectionString)
           // --- AJOUTER CETTE LIGNE POUR VOIR LES REQUÊTES SQL ---
...
{
    // Politique par défaut (requiert un token JWT, pour le Web)
    options.DefaultPolicy = new AuthorizationPolicyBuilder()
        .RequireAuthenticatedUser()
        .AddAuthenticationSchemes(JwtBearerDefaults.AuthenticationScheme)
        .Build();
    // Notre nouvelle politique "hybride"
