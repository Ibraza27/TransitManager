...
namespace TransitManager.Infrastructure.Services
{
    public class ConteneurService : IConteneurService
    {
        ...
        public async Task RecalculateStatusAsync(Guid conteneurId)
        {
            ...
            await UpdateAndSaveConteneurStatus(conteneur, _context);
        }

        public async Task<Conteneur> UpdateAsync(Conteneur conteneurFromUI)
        {
            var conteneurInDb = await _context.Conteneurs
                .Include(c => c.Colis)
                .Include(c => c.Vehicules)
            ...
            _context.Entry(conteneurInDb).CurrentValues.SetValues(conteneurFromUI);

            _context.Entry(conteneurInDb).Property("RowVersion").OriginalValue = conteneurFromUI.RowVersion;

            await UpdateAndSaveConteneurStatus(conteneurInDb, _context, oldStatus);
            if (conteneurInDb.NumeroPlomb != conteneurFromUI.NumeroPlomb)
            {
                await UpdatePlombOnChildren(conteneurInDb.Id, conteneurInDb.NumeroPlomb, _context);
            }
        ...
        public async Task<IEnumerable<Conteneur>> GetActiveAsync()
        {
            ...
            return await _context.Conteneurs.Where(c => c.Actif && !inactiveStatuses.Contains(c.Statut)).ToListAsync();
        }

        public async Task<IEnumerable<string>> GetAllDestinationsAsync()
        {
            return await _context.Conteneurs.Where(c => c.Actif).Select(c => c.Destination).Distinct().OrderBy(d => d).ToListAsync();
        }
        ...
        public async Task<IEnumerable<Conteneur>> GetOpenConteneursAsync()
        {
            ...
            return await _context.Conteneurs.Where(c => c.Actif && openStatuses.Contains(c.Statut)).ToListAsync();
        }

        public async Task<Conteneur?> GetByNumeroDossierAsync(string numeroDossier)
        {
            return await _context.Conteneurs.FirstOrDefaultAsync(c => c.NumeroDossier == numeroDossier);
        }

        public async Task<IEnumerable<Conteneur>> GetByDestinationAsync(string destination)
        {
            return await _context.Conteneurs.Where(c => c.Destination == destination).ToListAsync();
        }

        public async Task<IEnumerable<Conteneur>> GetByStatusAsync(StatutConteneur statut)
        {
            return await _context.Conteneurs.Where(c => c.Statut == statut).ToListAsync();
        }
